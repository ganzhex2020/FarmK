 ...
 */
class GcDetailAdapter:BaseQuickAdapter<AccountDetailEntity,BaseViewHolder>(R.layout.item_gcdetail),LoadMoreModule {
    companion object{
        var type = 0
    }
    ...
    override fun convert(holder: BaseViewHolder, item: AccountDetailEntity) {
        ...
        when(type){
            ...
            2->{
                tv_type.text = context.getString(R.string.fund_lc)
                iv_type.background = ContextCompat.getDrawable(context,R.drawable.bg_cricle_fundsub)
                iv_type.setImageDrawable(ContextCompat.getDrawable(context,R.drawable.ic_funddetail_lcf))
            }
        }
        ...
        try {
            if (type==0){
                ...
                tv_name.text = context.getString(TradItem.getTradItemLabel(item.tradeItem))
                tv_status.visible()
                tv_status.text = context.getString(TradState.getTradStateLabel(item.tradeType))
            }
        }catch (e :Exception){
        }
