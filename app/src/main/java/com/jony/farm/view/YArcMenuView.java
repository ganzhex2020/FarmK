...
public class YArcMenuView extends FrameLayout implements View.OnClickListener {
    ...
    private void handleStyleable(Context context, AttributeSet attrs, int defStyle) {
        ...
        try {
            ...
            mDuration = ta.getInteger(R.styleable.ArcMenuView_durations, 1000);
            mCanRotate = ta.getBoolean(R.styleable.ArcMenuView_can_rotate, true);
            mMenuResId = ta.getResourceId(R.styleable.ArcMenuView_menu_drawables, R.mipmap.ic_farm_fodder);
            mIsOpen = ta.getBoolean(R.styleable.ArcMenuView_initIsopen, false);
    ...
    private void initMenuView(Context context) {
        ...
        LayoutParams params = new LayoutParams(mMenuWidth, mMenuWidth);
        params.bottomMargin = 0;//mMenuItemWidth / 2;
        params.rightMargin = 0;//mMenuItemWidth / 2;
    ...
    private void initMenuItemViews(Context context) {
        ...
        for (int index = 0; index < mMenuItemResIds.size(); index++) {
            ...
            LayoutParams params = new LayoutParams(mMenuItemWidth, mMenuItemWidth);
            params.bottomMargin = 0;//mMenuItemWidth / 2;
            params.rightMargin = 0;//mMenuItemWidth / 2;
            params.gravity = Gravity.BOTTOM;
    ...
    //    mWidth = mMenuItemWidth*2 + mMenuWidth;
        mHeight = mMenuItemWidth*2 + mRadius;
     //   mHeight = mMenuItemWidth*2 + mMenuWidth;
